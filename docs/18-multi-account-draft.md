...
- **真机复验（2026-09-12，谷雨）**：装上 beta.2 后**绑定不再报错**，她口头确认"现在看好像是 ok 的"。
  这是唯一一条已由真机确认的结论；**下面这些仍未验**（`18-manual-test-plan.md` §4 的 B4/D1/D3/F1/F2）：
  两个账户是否真的各写各的树、同树拦截是否真的生效、两条长轮询并发是否不重不漏。
  注意这几项**失败时不会报错**（只会悄悄写进同一棵树 / 丢消息），所以必须按清单主动验，不能靠"看着正常"。
