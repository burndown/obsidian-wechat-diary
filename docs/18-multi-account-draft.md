...
- **真机验证（2026-09-12，谷雨）**：
  ...
  - ✅ **同树拦截第一层（测试项 D1）**：把账户 2 的文件夹改成账户 1 的 `黑高日记` → **被拦下**
    （提示点名「账户 1」、输入框回退、没有保存）。这一处是变异验证确认"自动测没兜住"的地方，
    也是唯一的**防数据混写**闸门，由真机确认的价值最大。
  - ✅ **历史文件的 author 用 `tools/backfill-author.py` 补齐**（谷雨报告"改完了"，未记录具体条数）。
    她的库**全在本机、没有云同步** —— 所以不存在"一批 mtime 变更引发同步冲突副本"的风险，
    跑前也用 `cp -R` 备过份，回滚成本就是恢复那两个文件夹。
    脚本自带 `--selftest`（20 条，含"演习不写盘""已有 author 不覆盖""CRLF 不混合行尾"），
    我这边跑过全通过；**脚本逻辑未走对抗审稿轮**。
- **仍未验**（`18-manual-test-plan.md` §4）：**D3 同树拦截第二层**（folder 不同、但用各自 pathFormat
  渲染出的当天路径相同）、B5 各账户段数各算各的、C 组隔离（提醒/撤回/称呼）、F1/F2 并发与离线补收、
